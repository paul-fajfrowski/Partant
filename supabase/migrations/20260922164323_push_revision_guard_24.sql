...
create or replace function public.product_push_settings(p_actor uuid,p_session uuid,p_action text,p_input jsonb default '{}') returns jsonb
...
begin
 ...
 if p_action='remove' then
  result:=public.product_push_device(p_actor,p_session,'status',input);
  delete from private.push_devices where actor=p_actor and session_id=p_session and id=(input->>'device')::uuid;
  result:=jsonb_set(result,'{registered}','false'::jsonb);
 else
 ...
 if p_action='preferences' then
  ...
  update private.product_revision set version=version+1, maintained_at=null where id=true;
 end if;
