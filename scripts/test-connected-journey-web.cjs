...
async function sessionFor(u) {
  ...
  assert.ok(s.access_token, "QA session");
  return s;
}
...
(async () => {
  try {
    const client = await sessionFor(users[1]),
      coach = await sessionFor(users[0]);
    const initial = await read(client),
      profile = initial.extraCoaches.find((c) => c.id === users[0].id);
    assert.ok(profile, "QA coach is published");
    await open(client);
    await until(
      () => H.d.body.textContent.includes("On bouge quand ?"),
      "Client home",
    );
    await H.click("Demain");
    await until(
      () => H.d.body.textContent.includes(profile.name),
      "Coach discovery",
    );
    ...
    ok(
      H.d.body.textContent.includes("Un moment pour vous."),
      "Live booking setup reached",
    );
    await H.click("Continuer");
    ok(
      H.d.body.textContent.includes("Vous y êtes presque."),
      "Live checkout reached",
    );
    await H.click("Confirmer ma réservation de test");
    await until(
      () => H.d.body.textContent.includes("Vous êtes partant."),
      "Server booking confirmation",
    );
    const current = await read(client),
      booking = current.bookings.find(
        (b) => !initial.bookings.some((old) => old.id === b.id),
      );
    ok(
      !!booking && booking.price === 50,
      "UI booking persisted once at correct price",
    );
    ok(
      current.bookings.length === initial.bookings.length + 1,
      "One UI confirmation creates one booking",
    );
    await H.click("Voir ma séance");
    ok(
      H.d.body.textContent.includes("Modifier ma séance"),
      "Booking detail manageable",
    );
    // Verify the second actor receives the same booking before opening its interface.
    const coachState = await read(coach);
    ok(
      coachState.bookings.some((b) => b.id === booking.id),
      "Coach receives client booking",
    );
    ok(
      coachState.notices.some((n) => n.booking === booking.id),
      "Coach receives booking notice",
    );
    ok(H.errors.length === 0, "Client has no runtime errors");
    await open(coach);
    await until(() => H.d.body.textContent.includes("Agenda"), "Coach home");
    const label = [...H.d.querySelectorAll('[role="button"]')]
    ...
    ok(
      H.d
        ...
        ?.textContent.includes(booking.clientName),
      "Booking notification rendered in coach inbox",
    );
    H.d.querySelector(`[data-testid="notification-${notice.id}"]`).click();
    await H.wait();
    await until(
      () => H.d.body.textContent.includes(booking.time),
      "Notification opens booking detail",
    );
    await until(
      async () =>
        (await read(coach)).notices.find((n) => n.id === notice.id)?.read,
      "Read notice persists",
    );
    ok(true, "Coach opens the booking and marks its notice as read");
    ok(H.errors.length === 0, "Coach has no runtime errors");
    console.log(
      `PASS ${checks} connected client/coach journey DOM checks with deployed Supabase. No real payment or visual validation.`,
    );
  } finally {
    if (H) H.close();
