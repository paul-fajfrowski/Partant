...
run("state.chatClient='alex'");submit('coach-message-form',{text:'À très vite'});equal(run("cfg.coachMessages.alex[0].text"),'À très vite');
...
run("state.entryRole='coach';state.entryMode='signup'");submit('code-form',{code:'123456'});equal(run('state.screen'),'coach-wizard');equal(run('cfg.published'),false);equal(run('publicationReady()'),false);run("act('publish',{})");equal(run('cfg.published'),false,'Incomplete coach cannot publish');
